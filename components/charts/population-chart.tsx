      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-lg min-w-[240px]">
        ...
        <div className="space-y-2">
          <p className="text-sm text-slate-700 dark:text-slate-300">
            <span className="font-semibold text-cyan-500">Población:</span>{' '}
            <span className="font-mono font-bold">{formatNumber(data.poblacion)}M</span>
          </p>
          {data.produccion && (
            <p className="text-sm text-slate-700 dark:text-slate-300">
...
export default function PopulationChart({ data }: PopulationChartProps) {
  ...
  // Data is already sorted by population in the parent component
    ...
    return (
      <Card className="p-6 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Población por País</h3>
        </div>
